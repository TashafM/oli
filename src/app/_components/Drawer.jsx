...
/**
 * Drawer (reusable, JS) — no collapsible menu
 * Props:
 *  - open (bool) controlled state
 *  - onClose (fn)
 *  - side ("left" | "right") default: left
 *  - widthClass (Tailwind width) default: "w-64"
 *  - title (string) optional, used for aria-labelledby
...
export function Drawer({
  ...
  const headingIdRef = useRef(`drawer-${Math.random().toString(36).slice(2)}-title`);
  ...
  const baseTranslate = side === "right" ? "translate-x-full" : "-translate-x-full";
  ...
  return createPortal(
    <>
      ...
      <div
        aria-hidden="true"
        className={`fixed inset-0 z-40 bg-black/30 transition-opacity ${open ? "opacity-100" : "opacity-0 pointer-events-none"} ${overlayClassName}`}
      ...
      <div
        ...
        className={`fixed top-0 drawer-bg  ${side === "right" ? "right-0" : "left-0"} z-50 h-screen p-4 overflow-y-auto bg-white dark:bg-gray-800 transform transition-transform duration-300 ${widthClass} ${open ? openTranslate : baseTranslate} ${className}`}
        tabIndex={-1}
      >
        {/* Close button */}
        <button
        ...
        {title ? (
          <h5 id={headingIdRef.current} className="text-base font-semibold text-gray-500 uppercase dark:text-gray-400">
            {
              'Olividya'
            }
          </h5>
...
/**
 * NavigationDrawerDemo — static menu (no collapsible)
 */
export default function NavigationDrawerDemo() {
  ...
  return (
    <div className="text-center p-6">
      ...
      {/* Drawer */}
      <Drawer open={open} onClose={() => setOpen(false)} side="left" widthClass="w-64">
